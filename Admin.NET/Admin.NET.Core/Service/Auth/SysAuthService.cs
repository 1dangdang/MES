...
public class SysAuthService : IDynamicApiController, ITransient
{
    ...
    /// <returns></returns>
    [AllowAnonymous]
    [DisplayName("账号密码登录（无需验证码）")]
    public virtual async Task<LoginOutput> LoginWithoutCaptcha([Required] LoginInput input)
    {
        // 判断密码错误次数（缓存30分钟）
        var keyPasswordErrorTimes = $"{CacheConst.KeyPasswordErrorTimes}{input.Account}";
        var passwordErrorTimes = _sysCacheService.Get<int>(keyPasswordErrorTimes);
        var passwordMaxErrorTimes = await _sysConfigService.GetConfigValue<int>(ConfigConst.SysPasswordMaxErrorTimes);
        // 若未配置或误配置为0负数, 则默认密码错误次数最大为10次
        if (passwordMaxErrorTimes < 1) passwordMaxErrorTimes = 10;
        if (passwordErrorTimes > passwordMaxErrorTimes) throw Oops.Oh(ErrorCodeEnum.D1027);
    ...
    /// <returns></returns>
    [AllowAnonymous]
    [DisplayName("简化登录（明文密码）")]
    public virtual async Task<LoginOutput> SimpleLogin([Required] string account, [Required] string password)
    {
        // 判断密码错误次数（缓存30分钟）
        var keyPasswordErrorTimes = $"{CacheConst.KeyPasswordErrorTimes}{account}";
        var passwordErrorTimes = _sysCacheService.Get<int>(keyPasswordErrorTimes);
        var passwordMaxErrorTimes = await _sysConfigService.GetConfigValue<int>(ConfigConst.SysPasswordMaxErrorTimes);
        // 若未配置或误配置为0负数, 则默认密码错误次数最大为10次
        if (passwordMaxErrorTimes < 1) passwordMaxErrorTimes = 10;
        ...
        // 获取登录租户和用户（使用默认租户）
        var (tenant, user) = await GetLoginUserAndTenant(null, account: account);

        // 账号是否被冻结
        if (user.Status == StatusEnum.Disable) throw Oops.Oh(ErrorCodeEnum.D1017);

        // 验证明文密码
        VerifyPasswordPlainText(password, keyPasswordErrorTimes, passwordErrorTimes, user);
    ...
    /// <param name="passwordErrorTimes"></param>
    /// <param name="user"></param>
    private void VerifyPasswordPlainText(string password, string keyPasswordErrorTimes, int passwordErrorTimes, SysUser user)
    {
        try
        {
            // 直接使用明文密码验证，不进行SM2密
